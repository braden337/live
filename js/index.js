...
function loadXml(callback) {
  $.ajax({
    type: "GET",
    url: "http://americanmotocrosslive.com/xml/" + series[0] + "/RaceResultsWeb.xml",
    dataType: "xml",
...
function getTheStuff() {
  loadXml(function(xml) {
    ...
    // }
    var $race = $(xml).find('A');
    var motoEvent = $race.attr('E').substring(0, $race.attr('E').indexOf('from')-1)
    var motoClass = $race.attr('S')
    var $results = $(xml).find('B');
    var practice = motoClass.indexOf('Qualifying') > -1;
    var time = practice ? 0 : 1;
    var times = ['Best', 'Gap'];
    var timesXML = ['BL', 'D']

    eventinfo.innerHTML = '<h3>' + motoEvent +
                          '</h2><h4>' + motoClass + '</h4>';

    var table = '<table class="table table-striped table-condensed">' +
                '<thead><tr><th class="center">Pos.</th>' +
                '<th class="right">#</td><th>Rider</th>' +
                '<th class="right">' + times[time] + '</th></tr></thead><tbody>';
    ...
    $.each($results, function(position, result) {
      table += '<tr><td class="center">' +
                ...
                $(result).attr('F').substring(3) +

                '</em></td><td class="right">' +

                $(result).attr(timesXML[time]) +
